...
class ImageProcessor:  # 1단계: 클래스 이름 정의
    ...
    def create_trackbar_init(self, cv_img):
        ...
        cv2.createTrackbar("Low_H", self.cropped_window, 75, 255, hsv_track)  # H의 최소 임계 값 트랙바 생성
        cv2.createTrackbar("Low_S", self.cropped_window, 0, 255, hsv_track)  # S의 최소 임계 값 트랙바 생성
        cv2.createTrackbar("Low_V", self.cropped_window, 118, 255, hsv_track)  # V의 최소 임계 값 트랙바 생성
        cv2.createTrackbar("Up_H", self.cropped_window, 255, 255, hsv_track)  # H의 최대 임계 값 트랙바 생성
