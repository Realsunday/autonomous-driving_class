...
class OBS_Control:  
    ...
    def search_right_wall(self):
        ...
        self.cmd_msg.angular.z = -0.2
        self.pub.publish(self.cmd_msg)
        print('Searching for the right wall')
    ...
    def control(self):
        ...
        if self.state == 'LEFT_WALL_FOLLOW':
            ...
            if self.obsL:
                ...
                print("LEFT_TURN 상태입니다")
                self.control_left_turn(5)  # 5초 동안 왼쪽으로 턴
                self.state = 'RIGHT_WALL_FOLLOW'

            elif self.state == 'RIGHT_WALL_FOLLOW':
                print("RIGHT_WALL_FOLLOW 상태입니다")
                if self.obsR:
                    print("오른쪽에 장애물이 있습니다")
                    self.control_right()
                else:
                    print("오른쪽에 장애물이 없습니다")
                    self.state = 'SEARCH_RIGHT_WALL'

            elif self.state == 'SEARCH_RIGHT_WALL':
                print("SEARCH_RIGHT_WALL 상태입니다")
                if not self.obsR:
                    print("오른쪽 벽을 찾고 있습니다")
                    self.search_right_wall()
                else:
                    print("오른쪽에 벽이 있습니다")
                    self.state = 'RIGHT_WALL_FOLLOW'

        else:
            print("기본 상태로 복귀합니다: LEFT_WALL_FOLLOW")
            self.state = 'LEFT_WALL_FOLLOW'
