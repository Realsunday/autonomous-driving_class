...
class move_limo:
    ...
    def drive_control(self):
        try:
            drive = Twist()
            if self.white_ratio >= 50:
                drive.linear.x = self.BASE_SPEED * 1.5
                drive.angular.z = self.BASE_ANGLE
            else:
                drive.linear.x = self.BASE_SPEED * 0.5  # white_ratio가 50 미만일 때 속도 저하

            if self.obs_dist < 20:
                drive.linear.x = 0
                print("stop")
            elif self.obs_dist < 50:
                drive.linear.x = max(drive.linear.x * 0.5, 0)  # 장애물 근접 시 추가 속도 저하
                print("speed down")
