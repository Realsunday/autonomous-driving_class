    ...
    def drive_control(self):
         ...
         try:
             ...
             if self.left_x is not None and self.right_x is not None:
                  #두 차선 모두 감지된 경우
                 combined_angle = (self.KP * (130 - self.left_x) + self.KP * (170 - self.right_x) / 2)
                 drive.angular.z = combined_angle
                 # print("Combined steering angle:", combined_angle)

             elif self.right_x is not None:
                  #오른쪽 차선만 감지된 경우
                 self.white_ANGLE = self.KP * (180 - self.right_x)
                 ...
                 # No lane detected
                 drive.linear.x = self.BASE_SPEED * 0.5  # Reduce linear velocity by 90%
