...
class ImageProcessor:  # 1단계: 클래스 이름 정의
    ...
    def create_trackbar_init(self, cv_img):
        ...
        cv2.createTrackbar("Low_V_1", self.cropped_window, 70, 255, hsv_track_group1)  # V의 최소 임계 값 트랙바 생성
        cv2.createTrackbar("Up_H_1", self.cropped_window, 46, 255, hsv_track_group1)  # H의 최대 임계 값 트랙바 생성
        ...
        # 두 번째 트랙바 그룹 생성
        cv2.createTrackbar("Low_H_2", self.cropped_window, 97, 255, hsv_track_group2)
        cv2.createTrackbar("Low_S_2", self.cropped_window, 0, 255, hsv_track_group2)
        cv2.createTrackbar("Low_V_2", self.cropped_window, 108, 255, hsv_track_group2)
    ...
    def run(self):
        ...
        if self.cam_flag:
            ...
            # 이 후 이미지 분석 로직 수행
            croped_img1, croped_img_shape1 = self.crop_img(hsv_img1)  # 첫 번째 그룹에 대한 영역 추출
            croped_img2, croped_img_shape2 = self.crop_img(hsv_img2)  # 두 번째 그룹에 대한 영역 추출
            
            binary_img1 = self.binary(croped_img1)
            binary_img2 = self.binary(croped_img2)

            if self.L_R_Value == 1: 
                pos_lane = self.calc_left_distance(croped_img1)
            else:
                pos_lane = self.calc_left_distance(croped_img1) + cv_img.shape[1] // 2
